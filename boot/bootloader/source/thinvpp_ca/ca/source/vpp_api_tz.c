 ...
 *  pass VBUF_INFO by means of TEEC_PARAMETERS
 *  split VBUF_INFO into subunit big enough to pass by TEEC_PARAMETERS
 *  requires alteast 4 to 5 transaction of PASSPAR to pass VBUF_INFO
 *
 ...
 *         MV_VPP_EBADCALL - function called previously
 **********************************************************/
INT MV_VPP_Init(VPP_INIT_PARM *vpp_init_parm)
{
...
/********************************************************************************
 * FUNCTION: Configure and initialise the panel
 * INPUT: handle - VPP object handle
 *        pConfigParams - parameter to configure the Panel
 * RETURN: MV_VPP_OK - SUCCEED
 *         MV_EBADPARAM - invalid parameters
 ********************************************************************************/
int MV_VPPOBJ_LoadMipiConfig(int handle, VPP_MIPI_LOAD_CONFIG *pConfigParams)
{
    HRESULT Ret = MV_VPP_OK;
    ...
    Ret = VPP_InvokePassShm_Helper(pConfigParams, VPP_MIPI_CONFIG, sizeof(VPP_MIPI_LOAD_CONFIG));
    return (Ret);
}
                        ...
                        VPP_DISP_OUT_PARAMS *pDispParams)
{
    ...
    Ret = VPP_InvokePassShm_Helper(aDispParamsData, VPP_SETFORMAT, VPP_SETFORMAT_SIZE);
