...
int VppInitialize(void)
{
    ...
    for (index = 0; index < MAX_TAVPP_INSTANCE_NUM; index++)
    {
        ...
        shmContext = &(TAVPPInstance[index].shmContext);

        /*create Pass sharemeory for each VPP TA instance*/
        memset(shmContext, 0, sizeof(TAVPPPassShm));
        result = VPP_ALLOC(0, TAVPP_PASS_SHMSIZE, 32, &shmContext->shm_handle);
        if (result != SUCCESS) {
            printf("VPP pass share memory malloc failed %x\n", result);
            return -1;
        }
        shmContext->vir_addr = (unsigned int*)(uintptr_t)shmContext->shm_handle;
...
#ifdef VPP_ENABLE_TZMEM_DYNAMIC
    pTzShmMem = malloc(sizeof(TEEC_SharedMemory));
...
int VPP_InvokePassShm_Helper(void *pBuffer, VPP_SHM_ID shmCmdId, UINT32 sBufferSize)
{
    HRESULT Ret;
    TAVPPPassShm *shmContext;
    shmContext = VppSelectPassShm();

    memcpy(shmContext->vir_addr, pBuffer, sBufferSize);
    flush_dcache_range(shmContext->vir_addr, (void *)(((char *)shmContext->vir_addr) + sBufferSize));
    Ret = VppPassShm(shmContext->vir_addr, shmCmdId, sBufferSize);
