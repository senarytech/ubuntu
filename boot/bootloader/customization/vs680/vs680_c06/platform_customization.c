...
{
	printf("set hdmi tx and rx power %d, lhl\n",value);
	GPIO_PortWrite(38,value);
...
static void update_pmic(void *fdt)
{
	...
	if (vcpu_pmic_id != default_pmic_vcpu) {
		...
		else if(SYR837_8 == vcpu_pmic_id) {
			/* enable SY8827N */
			offset = fdt_node_offset_by_compatible(fdt, -1, "silergy,syr837_8");
			while(offset != -FDT_ERR_NOTFOUND) {
				const char *name = fdt_getprop(fdt, offset, "regulator-name", NULL);
				if(name && !strcmp(name, "vcpu")) {
					uint32_t phandle = fdt_get_phandle(fdt, offset);
				...
				offset = fdt_node_offset_by_compatible(fdt, offset, "silergy,syr837_8");
			}
		}
		else
	...
	if (vcore_pmic_id != default_pmic_vcore) {
		...
		/* enable SYR837_8 */
		else if(SYR837_8 == vcore_pmic_id) {
			offset = fdt_node_offset_by_compatible(fdt, -1, "silergy,syr837_8");
			while(offset != -FDT_ERR_NOTFOUND) {
				const char *name = fdt_getprop(fdt, offset, "regulator-name", NULL);
				if(name && !strcmp(name, "vcore")) {
...
static int __attribute__((used)) plat_bootloader_initchip_stage(void)
{
	...
	else if(0x2 == board_version) {
		NOTICE("INFO: STB Revision 2.0(3'b010) Board Detected!\n");
		set_pmic_id(SYR837_8, SYR837_8);
	}
...
GPIO_PortWrite(13, 1);
// hdmi rx hpd
SM_GPIO_PortWrite(12, 1);
//usb id
//
SM_GPIO_PortWrite(15, 0);
// rtl8363 reset
SM_GPIO_PortWrite(17, 1);
return BOOTLOADER_NULL_STAGE;
}
