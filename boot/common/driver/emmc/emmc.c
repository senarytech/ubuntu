...
extern int do_emmcinit(enum mmc_id id);
...
static int init_emmc(enum mmc_id id)
{
	int ret = 0;
	ret = do_emmcinit(id) ;
	if (ret) {
		ERR("mmc init fail,id %d\n", id);
		//FIXME
		//reset_soc();
	}
	return ret;
...
static long long adaptor_get_capacity(enum flash_id mmc_id)
{
	if(flash_mmc0 == mmc_id){
