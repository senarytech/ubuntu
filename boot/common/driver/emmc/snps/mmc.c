...
//scr register is 64bit,[51:48] indicate the SD_BUS_WIDTHS
//[48] indicate 1bit support,[50] indicate 4bit support
...
static int mmc_sd_switch(unsigned int bus_width)
{
	...
	scr[1] = __be32_to_cpu(scr[1]);

	dbg("scr[0], 0x%x", scr[0]);
	dbg("scr[1], 0x%x", scr[1]);

	//parse SCR for SD version v1.0 or don't support 4bit,then return
	if ((scr[0] & SD_SCR_BUS_WIDTH_4) == 0U) {
		printf("card don't support 4bit\n");
		return 0;
	}
...
static int mmc_enumerate(unsigned int clk, unsigned int bus_width, struct mmc_device_info *device_info)
{
	int ret;
	unsigned int resp_data[4];
	unsigned int csd_version = 0;
	ret = ops->init(device_info);
	...
	if (mmc_dev_info->mmc_dev_type == MMC_IS_EMMC) {
		...
		//TODO: Switching voltage to 1.8v
	}
	if (ret != 0) {
		ERR("cmd8 error, %s, %d, ret = 0x%x\n",__func__, __LINE__, ret);
		return ret;
	}
	...
	if (mmc_dev_info->mmc_dev_type == MMC_IS_EMMC) {/* CMD3: For MMC card Set Relative Address */
		rca = MMC_FIX_RCA;
	...
	} else {/* CMD3: For SD card Get Relative Address */
...
size_t mmc_rpmb_write_request(int lba, const uintptr_t buf, size_t size)
{
	int ret;
	unsigned int cmd_idx, cmd_arg;
	     ...
	     struct mmc_device_info *device_info)
{
	...
	return mmc_enumerate(clk, width, device_info);
}
