...
#define EXPANDER_GPIO2_PORT_DEV_SWT_OEn  0
...
static int dwcmshc_init(struct mmc_device_info *device_info);
...
void emmc_reset(struct mmc_device_info *device_info)
{
	...
	if(MMC_IS_EMMC == device_info->mmc_dev_type) {
...
void set_controller_voltage(enum mmc_power mmc_power)
{
	unsigned int val;
	if(MMC_1V8 == mmc_power) {
...
void phy_config(enum mmc_power mmc_power)
{
	unsigned int val, count, i;
	if(MMC_1V8 == mmc_power) {
		/* config PHY_CNFG, general configuration */
		val = mmc_readl(pad_general_1v8.addr);
		...
		for(i=0; i<5; i++)
		{
			...
			mmc_writel(val, pad_txslew_ctrl_p_1v8[i].addr);

			/* config PHY TXSLEW_CTRL_N */
			val = mmc_readl(pad_txslew_ctrl_n_1v8[i].addr);
			...
			mmc_writel(val, pad_txslew_ctrl_n_1v8[i].addr);
		}
	}
	else if(MMC_3V3 == mmc_power) {

		/* config PHY_CNFG, general configuration */
		val = mmc_readl(pad_general_3v3.addr);
		...
		mmc_writel(val, pad_general_3v3.addr);

		for(i=0; i<5; i++)
		{
			/* config PHY RXSEL */
			val = mmc_readl(pad_rxsel_3v3[i].addr);
			val &= ~(pad_rxsel_3v3[i].mask<<pad_rxsel_3v3[i].bit);
			val |= (pad_rxsel_3v3[i].value<<pad_rxsel_3v3[i].bit);
			mmc_writel(val, pad_rxsel_3v3[i].addr);

			/* config PHY WEAKPULL_EN */
			val = mmc_readl(pad_weakpull_en_3v3[i].addr);
			val &= ~(pad_weakpull_en_3v3[i].mask<<pad_weakpull_en_3v3[i].bit);
			val |= (pad_weakpull_en_3v3[i].value<<pad_weakpull_en_3v3[i].bit);
			mmc_writel(val, pad_weakpull_en_3v3[i].addr);

			/* config PHY TXSLEW_CTRL_P */
			val = mmc_readl(pad_txslew_ctrl_p_3v3[i].addr);
			...
			mmc_writel(val, pad_txslew_ctrl_p_3v3[i].addr);

			/* config PHY TXSLEW_CTRL_N */
			val = mmc_readl(pad_txslew_ctrl_n_3v3[i].addr);
			...
			mmc_writel(val, pad_txslew_ctrl_n_3v3[i].addr);
		}
	}
	else {
...
void phy_deassert_reset(unsigned int mmc_dev_type)
{
	unsigned int val = 0;

	/* deassert global PHY sticky reset */
	phy_deassert_global_reset(mmc_dev_type);
...
void emmc_init(struct mmc_device_info *device_info)
{
	unsigned int val;
	...
	if(MMC_IS_EMMC == device_info->mmc_dev_type)
		...
		phy_config(MMC_3V3);
	phy_deassert_reset(device_info->mmc_dev_type);
	/* From DIAG */
	phy_delayline_setup(EMMC_TX_DELAY_CODE, 0);
	phy_tuning_setup();
#endif
	if(MMC_IS_EMMC == device_info->mmc_dev_type)
		set_controller_voltage(MMC_1V8);
	else
		set_controller_voltage(MMC_3V3);

	dwcmshc_host_clock_setup(SDCLK_SEL_DIV_1024);

#if 0
//#ifdef CONFIG_BOOT_SPI_THEN_SD
		REG_READ32(MEMMAP_CHIP_CTRL_REG_BASE + BA_Gbl_perifStickyResetN_sdioPhyRstn, &val);
		val |= MSK32Gbl_perifStickyResetN_sdioPhyRstn;
		REG_WRITE32(MEMMAP_CHIP_CTRL_REG_BASE + BA_Gbl_perifStickyResetN_sdioPhyRstn, val);
		REG_READ32(MEMMAP_CHIP_CTRL_REG_BASE + BA_Gbl_perifStickyResetN_sdioPhyRstn, &val);
		dbg("StickyResetN is 0x%x", val);
#endif
...
void emmc_isr()
{
	unsigned int int_stat, int_err_stat, val1, i, complete = 0;
...
void emmc_intr_on(struct mmc_device_info *device_info)
{
	if(register_isr(emmc_isr, device_info->irq))
		ERR("mmc isr register failed\n");
	set_irq_enable(device_info->irq);
}

static int dwcmshc_init(struct mmc_device_info *device_info)
{
	int ret = 0;
	emmc_reset(device_info);
	emmc_init(device_info);
	emmc_intr_on(device_info);
	ret =  emmc_wait_card_detect();
...
#ifdef CONFIG_BOOT_SPI_THEN_SD
static int pinmux_write_soc(unsigned int cntl_bus_addr, unsigned int mask, unsigned int lsb, unsigned int mode)
{
	unsigned int pinmux;
...
int do_emmcinit(enum mmc_id id)
{
	...
	if(mmc1 == id) {
		...
		ptr_params->flags = MMC_FLAG_CMD23; //set block count for UHS104 card
		ptr_info->mmc_dev_type = MMC_IS_SD;
		ptr_info->ocr_voltage = OCR_2_9_3_0 | OCR_3_0_3_1| OCR_3_2_3_3 | OCR_3_3_3_4;
		ptr_info->clk_en = LSb32Gbl_clkEnable_sdioSysClkEn;
		ptr_info->MSKsyncReset = MSK32Gbl_perifReset_sdioSyncReset;
		ptr_info->syncReset = LSb32Gbl_perifReset_sdioSyncReset;
		ptr_info->irq = IRQ_sdio_interrupt;

		//SDIO signal mux to sd card
		pinmux_write_soc(RA_Gbl_pinMuxCntlBus, MSK32Gbl_pinMuxCntlBus_TW0_SCL, LSb32Gbl_pinMuxCntlBus_TW0_SCL,
			Gbl_pinMuxCntlBus_TW0_SCL_MODE_1);
		pinmux_write_soc(RA_Gbl_pinMuxCntlBus1, MSK32Gbl_pinMuxCntlBus_TW0_SDA, LSb32Gbl_pinMuxCntlBus_TW0_SDA,
			Gbl_pinMuxCntlBus_TW0_SDA_MODE_1);
		ret = i2c_fxl6408_gpio_write(SOC_I2C0, EXPANDER_GPIO_0, EXPANDER_GPIO2_PORT_DEV_SWT_OEn, 1);
		if(ret)
			dbg("mux to sd card failed\n");
	} else {
		...
		ptr_info->clk_en = LSb32Gbl_clkEnable_emmcSysClkEn;
		ptr_info->MSKsyncReset = MSK32Gbl_perifReset_emmcSyncReset;
		ptr_info->syncReset = LSb32Gbl_perifReset_emmcSyncReset;
		ptr_info->irq = IRQ_emmc_int;
	}
#else
	ptr_params->reg_base = MEMMAP_EMMC_REG_BASE;
	ptr_params->clk_rate = 50; // 50MHz: high speed
	...
	NOTICE("MMC is %s\n",ptr_info->mmc_dev_type == MMC_IS_EMMC ? "EMMC" : "SD");
