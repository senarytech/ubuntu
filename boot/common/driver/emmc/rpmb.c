...
int rpmb_get_write_count(void)
{
	...
	ret = mmc_write_blocks(0, (uintptr_t)frame_in, sizeof(rpmbFrame));
	if (0 != ret) {
		ERR("RPMB: mmc failed to write block!!!\n");
		goto out1;
	}
	...
	ret = mmc_read_blocks(0, (uintptr_t)frame_in, sizeof(rpmbFrame));
	if (0 != ret) {
		ERR("RPMB: rpmb failed to read block!!!\n");
		goto out1;
	}
...
int rpmb_read_blocks(int blk_index, int blk_num, void *buff, size_t buff_len)
{
	...
	frame_in->blockCount = htobe16(blk_num);
	frame_in->writeCounter = 0;
	...
	ret = mmc_read_blocks(blk_index, (uintptr_t)data, MMC_BLOCK_SIZE * blk_num);
	if (0 != ret) {
		ERR("RPMB: rpmb failed to read block!!!\n");
		goto out1;
	}
...
int rpmb_write_blocks(int blk_index, int blk_num, void *buff, size_t buff_len)
{
	...
	if (write_count < 0)
	{
		ERR("RPMB: rpmb failed to get write count\n");
		ret = -1;
		goto out1;
	}
	...
	// send request frame
	ret = mmc_rpmb_write_request(blk_index, (uintptr_t)frame_req, sizeof(rpmbFrame));
	if (0 != ret) {
		ERR("RPMB: mmc failed to request!!!\n");
		goto out1;
	}
	...
	ret = mmc_write_blocks(blk_index, (uintptr_t)frame_req, sizeof(rpmbFrame));
	if (0 != ret) {
		ERR("RPMB: mmc failed to write block!!!\n");
		goto out1;
	}
	...
	mmc_read_blocks(blk_index, (uintptr_t)frame_req, sizeof(rpmbFrame));
	if (0 != ret) {
		ERR("RPMB: rpmb failed to get response!!!\n");
		goto out1;
	}
