 ...
 *
 * The bootdev_sel is written by user for force boot from sd card
 * 0 - default (if sd card is detect,boot from sd card
 * 				if sd card is not detect,boot from emmc)
 * 1 - boot from sd card only
 * 2 - boot from emmc only
 */
...
int get_bootdev_sel_from_boot_info(void);
...
void set_bootdev_sel_to_boot_info(int mode);
