...
#define SYR837_8_STEPuV 12500
#define MIN_VOLTAGE 712500
#define MAX_VOLTAGE 1500000

#define SYR837_SLAVE_ADDR 	0x40
#define SYR838_SLAVE_ADDR 	0x41

#define SLAVE_ADDR 	SYR837_SLAVE_ADDR
...
static int syr837_8_pmic_volt2data(int volt, int *p_data)
{
	unsigned int data;

	if ((volt >= MIN_VOLTAGE) && (volt <= MAX_VOLTAGE))
		data = (volt - MIN_VOLTAGE) / SYR837_8_STEPuV;
	else
		data = RSVD;
...
static int syr837_8_pmic_data2volt(int *p_volt, int vdata)
{
	unsigned int volt;
	...
	data = data & 0x3F;
	if (data <= 0x3f)
		volt = MIN_VOLTAGE + (data * SYR837_8_STEPuV);
	else
		volt = RSVD;
...
static int syr837_8_set_vol(int master_id, int slaveAddr, int volt)
{
	int target = volt;
	...
	ret = syr837_8_pmic_volt2data(target, &vdata);
	if (ret == 0) {
		...
		if (data_reg0 != data) {
			dbg_printf(PRN_ERR, "PMIC syr837_8: read:0x%02x != write:0x%02x\n", data, data_reg0);
			return -1;
		}
		mdelay(1);
	}
...
static int syr837_8_get_vol(int master_id, int slaveAddr)
{
        ...
        ret |= syr837_8_pmic_data2volt(&volt, (int)read);
        if(ret) {
...
{
	int volt = syr837_8_get_vol(VOUT_CPU_ID, SLAVE_ADDR);
...
{
	int volt = syr837_8_get_vol(VOUT_CORE_ID, SLAVE_ADDR);
...
static int i2c_set_volt(int master_id, int from, int to)
{
	...
	while (volt != to){
		...
		if(0 != syr837_8_set_vol(master_id, SLAVE_ADDR, volt))
			return 1;
	}
