...
//
//    // assert reset
//    BFM_HOST_Bus_Read32(APLL_SWRST_ADDR, &val);
//    val &= ~(1<<(pll+1));
//    val |= (rst^0x1)<<(pll+1);
//    BFM_HOST_Bus_Write32(APLL_SWRST_ADDR, val);
//
//    return ret;
//}
...
int diag_config_pll_getpfdrange(unsigned int pllbase){
	...
	r=7;
        ...
        dbg_printf(PRN_RES, "ILLEGAL input FPFD!!\n");
	r=0;
    }
    return r;
}
...
int diag_config_pll_ssc(unsigned int pllbase, int ssc_freq, int ssc_amp, int ssc_mode, int enable)
{
	...
	if(ssc_amp<=2){
        ...
        dbg_printf(PRN_INFO, "written SSC depth is: %.1f,  input SSC depth is: %.1f\n", wssc_amp, (double)ssc_amp/10);
    ...
    dbg_printf(PRN_INFO, "Calculated FPFD frequency is: %.2f\n", fpfd);
    dbg_printf(PRN_INFO, "Calculated SSCD frequency is: %.2f\n", ssc_freqd);
    dbg_printf(PRN_INFO, "Calculated SSC frequency is: %.2f\n", fpfd/ssc_div);
	// select SSMF base on SSC_DIV
	// binary search, closest
	ssfreq=bsearch(divs, ssc_div);
    dbg_printf(PRN_INFO, "Table Searched SSMF is: %d\n", ssfreq);
	ssmode=ssc_mode;
    }else{
        ssfreq = 0;
        ssdepth= 0;
...
int diag_check_feedback_only(unsigned int pllbase, unsigned int ndivr){
    ...
    double fpfd=diag_config_pll_getfpfd(pllbase);
    dbg_printf(PRN_INFO, "Calculated FPFD is: %.2f, Current FPFD is: %.2f\n", nfpfd, fpfd);
...
int diag_change_feedback_otf(unsigned int pllbase, unsigned int ndivr, unsigned int divfi, unsigned int divff){
	...
	if(diag_wait_pll_divack(pllbase, ENABLE)){
            dbg_printf(PRN_RES, "WAIT for DIVACK ASSERT TIME OUT!\n");
	    ret=-1;
	}
	// 5. de-assert newdiv
	...
	if(diag_wait_pll_divack(pllbase, DISABLE)){
            dbg_printf(PRN_RES, "WAIT for DIVACK DE-ASSERT TIME OUT!\n");
	    ret=-1;
	}
    }
    return ret;
        ...
        int ssc_amp, int ssc_mode, int calc, APLLCFG_t apll_cfg)
{
    ...
    if(calc)
    {
        ...
        if(vco>6000)
        {
            dbg_printf(PRN_RES, "INVALID VCO: %d\n", vco);
            ret = -1;
            return ret;
        }
	...
	{
            dbg_printf(PRN_RES, "INVALID DIVQ and DIVQF: %d %d\n", dp, dp1);
            ret = -1;
            return ret;
	}
	...
	dbg_printf(PRN_INFO, " calculated DIVFI is: %d\n", INT_N);
	dbg_printf(PRN_INFO, " calculated fraction is: %f\n", FRAC_N);
	dbg_printf(PRN_INFO, " calculated DIVFF is: %d\n", frac);
	dbg_printf(PRN_INFO, " calculated DIVQ and DIVQF is: %d %d\n", divq, divqf);
    }
    else
    {
        divr=apll_cfg.dm;
	...
	if(ret==-1)
#if TARGET != BOOT
	    dbg_printf(PRN_RES, "On-The-Fly feedback change handshaking issue happens!\n");
#else
	...
	if(ssc_ret != 0)
	    dbg_printf(PRN_RES, "SSC change fail, SSC config not changed!\n");
	...
	if(ret)
	{
	    //dbg_printf(PRN_RES, "PLL CHANGE FAIL!!!\nPLL in BYPASS mode!!!\n");
	    dbg_printf(PRN_RES, "PLL NOT LOCK!!!\nASSUME PLL IS READY!!\n");
	    //return ret;
	}
#if 0
    }
	...
	int ssc_mode, unsigned int calc, APLLCFG_t pll_cfg)
{
	...
	dbg_printf(PRN_RES, "INVALID input apll index!\n");
	return -1;
    }
