...
extern void emmc_phy_tx_delay(P_MM4_SDMMC_CONTEXT_T pContext, UINT_T tx_delay_code, int extdly_en);
...
UINT_T EMMCHC_Init(void)
{
	...
	//step 2~8
	// According to: VS680 DIAG hardware configuration note[030921], identification
	emmc_phy_tx_delay(pContext, 127, 0);
	//ZL: Dolphin_BG7_PHY_bring_up_sequence.xlsx
	//step 9
...
*/
	...
	// According to: VS680 DIAG hardware configuration note[030921], HS SDR50
	emmc_phy_tx_delay(pContext, 100, 0);
    return NoError;
}
...
int cmd_EMMC_RESET(void)
{
	...
	//EMMC reset PHY
	BFM_HOST_Bus_Read32(MEMMAP_CHIP_CTRL_REG_BASE + RA_Gbl_perifStickyResetN, &val);
	val &= ~MSK32Gbl_perifStickyResetN_emmcPhyRstn;
	BFM_HOST_Bus_Write32(MEMMAP_CHIP_CTRL_REG_BASE + RA_Gbl_perifStickyResetN, val);
	udelay(1);
	val |= MSK32Gbl_perifStickyResetN_emmcPhyRstn;
	BFM_HOST_Bus_Write32(MEMMAP_CHIP_CTRL_REG_BASE + RA_Gbl_perifStickyResetN, val);
...
#if 0
void cmd_EMMC_TEST( char *params_str)
{
    unsigned int cmdIdx;
    UINT_T argument, respond;
    UINT_T respBuf[4];
    // Initialize Flash Properties

    P_SDMMC_Properties_T pSDMMCP = EMMC_GetProperties();
    DIAG_PARAM_PARSER_INT(params_str, cmdIdx,     0);
    DIAG_PARAM_PARSER_INT(params_str, argument,     0);
    DIAG_PARAM_PARSER_INT(params_str, respond,     0);

    EMMC_SendSetupCommand(pSDMMCP, cmdIdx, MM4_CMD_TYPE_NORMAL, argument, respond);
    /*
    Retval = MM4_Interpret_Response(pSDMMCP, resp, 0x100);
    if (Retval != NoError)
    return SDMMCInitializationError;
    */
    EMMC_PRN(PRN_INFO,"\n TEST: send command: index:%d arg=0x%08x resp=0x%08x",cmdIdx, argument, respond);
    respBuf[0]=pSDMMCP->CardReponse.pBuffer[0];
    respBuf[1]=pSDMMCP->CardReponse.pBuffer[1];
    respBuf[2]=pSDMMCP->CardReponse.pBuffer[2];
    respBuf[3]=pSDMMCP->CardReponse.pBuffer[3];

    EMMC_PRN(PRN_INFO,"\n --> card respond: 0x%08x 0x%08x 0x%08x 0x%08x",respBuf[0],respBuf[1],respBuf[2],respBuf[3]);
}


/*************************
tuning for HS200 and HS400
*************************/

int cmd_EMMC_TUNE(char *params_str)
{
    UINT_T i=0,Retval=0,addr,val;
	uintptr_t ad = 0;
    ...
    pSDMMCP->Trans.LocalAddr = SDIO_RD_BUF;
    pSDMMCP->Trans.WordIndex = 0;
    // Must set MMC NUMBLK

	ad = (uintptr_t)pEMMCCTRL;
    addr= (uint32_t)ad;

    EMMC_PRN(PRN_DBG, "\n  Start tuning procedure...");

    BFM_HOST_Bus_Read32(addr+0x3C,&val);
    EMMC_PRN(PRN_DBG, "\n  Reg0x%x, value is 0x%x",addr+0x3C,val);
    val &= ~0xC00000;
    //write 0x0 to SAMPLING_CLK_SEL in Host Ctrl Reg2 to reset
    BFM_HOST_Bus_Write32(addr+0x3C,val);
    //write 0x1 to EXE_TUNING in Host Ctrl Reg2 to start tuning
    val |= 0x400000;
    BFM_HOST_Bus_Write32(addr+0x3C,val);
    BFM_HOST_Bus_Read32(addr+0x3C,&val);
    EMMC_PRN(PRN_DBG, "\n  Reg0x%x, value is 0x%x\n",addr+0x3C,val);

    for(i=0;i<40;i++)
    {
        Retval = EMMC_SendTuningFuncCmd();
        BFM_HOST_Bus_Read32(addr+0x3C,&val);
        val &= 0xC00000;
        EMMC_PRN(PRN_DBG, "\nAfter:  Reg0x%x, value is 0x%x;return code 0x%x\n",addr+0x3C,val,Retval);
        if(val == 0x400000)
        	continue;
        else if(val == 0x800000)
        {
        	EMMC_PRN(PRN_INFO, "Tuning Complete! iteration %d\n",i);
        	break;
        }
        else if(val == 0x0)
        {
...
int cmd_EMMC_TXCFG(char *params_str)
{
////	unsigned int addr,val,i,delay;
////
////	//Program the sdio3DllMstRefClk to be 100 MHz (MEMMAP_CHIP_REG_BASE + FA_Gbl_sdioDllMstCtrl (0x250))
////	//BFM_HOST_Bus_Read32(MEMMAP_CHIP_REG_BASE+sdioDllMstRef,&val);
////  	//val|=0x40000;
////  	BFM_HOST_Bus_Write32(MEMMAP_CHIP_CTRL_REG_BASE+RA_Gbl_sdioDllMstRef,0x229);
////
////	//Program the DLL Master Reset ( bit 22)  using the Global register using the following register addr (MEMMAP_CHIP_REG_BASE + FA_Gbl_sdioDllMstCtrl (0x254) ) to a value of 1 ( active high reset )
////	BFM_HOST_Bus_Read32(MEMMAP_CHIP_CTRL_REG_BASE+RA_Gbl_sdioDllMstCtrl,&val);
////  	val|=MSK32Gbl_sdioDllMstCtrl_RESET;//0x400000;
////  	BFM_HOST_Bus_Write32(MEMMAP_CHIP_CTRL_REG_BASE+RA_Gbl_sdioDllMstCtrl,val);
////
////	//Wait for atleast 10ns. Better to wait for 5 clks of sdio3MstRefClk to be conservative
////	for(i=0;i<10000;i++)
////	{
////		i+=1;i-=1;
////	}
////
////	//Program the PH_SEL4 to a value 0x1f ((MEMMAP_CHIP_REG_BASE + FA_Gbl_sdioDllMstCtrl (0x254) bits [19:15] of the register
////	BFM_HOST_Bus_Read32(MEMMAP_CHIP_CTRL_REG_BASE+RA_Gbl_sdioDllMstCtrl,&val);
////	val&=~MSK32Gbl_sdioDllMstCtrl_PH_SEL4;
////  	val|=(degree_phase_shift<<LSb32Gbl_sdioDllMstCtrl_PH_SEL4);//0xF8000
////  	EMMC_PRN(PRN_INFO, "\n  master Dll ctrl is 0x%x",val);
////  	BFM_HOST_Bus_Write32(MEMMAP_CHIP_CTRL_REG_BASE+RA_Gbl_sdioDllMstCtrl,val);
////
////	//Program the DLL Master Reset ( bit 22)  using the Global register using the following register addr (MEMMAP_CHIP_REG_BASE + FA_Gbl_sdioDllMstCtrl (0x254) ) to a value of 0 ( deassert reset )
////	BFM_HOST_Bus_Read32(MEMMAP_CHIP_CTRL_REG_BASE+RA_Gbl_sdioDllMstCtrl,&val);
////  	val&=(~MSK32Gbl_sdioDllMstCtrl_RESET);//~0x00400000
////  	BFM_HOST_Bus_Write32(MEMMAP_CHIP_CTRL_REG_BASE+RA_Gbl_sdioDllMstCtrl,val);
////
////	//Wait for atleast 10ns. Better to wait for 5 clks of sdio3MstRefClk (100MHz) to be conservative
////
////	//Wait for 4096 clocks of sdio3MstRefClk ( 100MHz as programmed in step1.) after step 6. Just to be conservative, wait extra 10 clocks(100MHz Clock). ( this is for DLL lock )
////	val=0;
////	while(val!=1)
////	{
////		BFM_HOST_Bus_Read32(MEMMAP_CHIP_CTRL_REG_BASE+RA_Gbl_sdioDllMstStatus,&val);
////		val=(MSK32Gbl_sdioDllMstStatus_DLL_LOCK&val)>>28;
////	}
////
////	//Read the status using the sdioDllMstStatus register. This is the delay value that needs to be programmed into the delay element of MMC controller
////	//Read MEMMAP_CHIP_REG_BASE + FA_Gbl_sdioDllMstCtrl (0x25c) and use the read data to program in  step9. As we programmed PH_SEL4, the status has to be read for DELAY_CTRL4 ( bits 26:18]
////
////	BFM_HOST_Bus_Read32(MEMMAP_CHIP_CTRL_REG_BASE+RA_Gbl_sdioDllMstStatus,&val);
////	EMMC_PRN(PRN_INFO, "\n  master Dll status is 0x%x",val);
////	delay=(val&MSK32Gbl_sdioDllMstStatus_DELAY_CTRL4)>>LSb32Gbl_sdioDllMstStatus_DELAY_CTRL4;
////	EMMC_PRN(PRN_INFO, "\n  master calibrated DLL delay is 0x%x",delay);
////	//Program the 9-bit  delay value in the MMC Device DLL Register in the MMC controller. The value is obtained from step8.
////	// Program the 9-bit delay value for MMC Device DLL register in the MMC controller; Programming the DLL slave
////	addr=((unsigned int)pSDCTRL[guchCardId])+SDIO_TX_CFG_REG_OFFSET;
////
////
////	BFM_HOST_Bus_Read32(addr,&val);
////  	val=val&0xFE00FE00|delay|delay<<16;
////  	EMMC_PRN(PRN_INFO, "\n Write 0x%x to TX DLL register at 0x%x",val,addr);
////  	BFM_HOST_Bus_Write32(addr,val);
////  	return 0;
	return 0;
}


int cmd_EMMC_VRSEC(char *params_str)
{
    UINT_T temp;
    unsigned int start_blk;
    unsigned int cnt;
    unsigned char *buf_rd;
    unsigned char *buf_wr;

    DIAG_PARAM_PARSER_HEX(params_str, start_blk,     0);
    DIAG_PARAM_PARSER_HEX(params_str, cnt,     0);
    DIAG_PARAM_PARSER_HEX(params_str, buf_rd,     0);
    DIAG_PARAM_PARSER_HEX(params_str, buf_wr,     0);

    int rtn,i,j;

    	EMMC_Get_Extcsd();
    EMMC_PRN(PRN_RES,"\nDevice block count (EXT_CSD[215:212]) is 0x%08X (capacity %d blocks)\n",g_device_sector_count,g_device_sector_count );
    ...
    if((start_blk+cnt)>g_device_sector_count)
    {
    	EMMC_PRN(PRN_RES,"\nExceeding device capacity. Test will timeout beyond the capacity !\n");
    }

    EMMC_PRN(PRN_RES,"\nSequential flash verification starting from block #%d, block count is %d\n\n",start_blk,cnt);
    for (i=0;i<cnt;i++)
    {
        //memset(buf_wr,0,512);//clear write buffer
        //read original data
        memset(buf_rd,0,512);//clear read buffer

        EMMC_PRN(PRN_INFO,"\n iteration %d block  M2M test ",start_blk);

        EMMC_PRN(PRN_INFO,"\n read data from block 0x%x ", start_blk);
        rtn=EMMC_ReadBlk((long long)start_blk*0x200, buf_rd, 0x200);

        //invert original data and write back
        memcpy(buf_wr,buf_rd,512);
        for(j=0;j<128;j++)
        {
        	temp=GET_P_UINT(buf_wr)[j];
        	*(unsigned int*)(GET_P_UINT(buf_wr)+j)=~temp;
        }

        EMMC_PRN(PRN_INFO,"\n write inverted data to block 0x%x ", start_blk);
        rtn=EMMC_WriteBlk((long long)start_blk*0x200, buf_wr, 0x200);

        //read inverted data and check
        memset(buf_rd,0,512);//clear read buffer
        EMMC_PRN(PRN_INFO,"\n read inverted data from block 0x%x ", start_blk);
        rtn=EMMC_ReadBlk((long long)start_blk*0x200, buf_rd, 0x200);

        for(j=0;j<128;j++)
        {
        	if(GET_P_UINT(buf_wr)[j]!=GET_P_UINT(buf_rd)[j])
        	{
        		EMMC_PRN(PRN_ERR,"Flash error at Sector #%d:\nbuf_wr[%d]=0x%08X,buf_rd[%d]=0x%08X\n",start_blk,j,GET_P_UINT(buf_wr)[j],j,GET_P_UINT(buf_rd)[j]);
        		return 1;
        	}
        }
        ...
        EMMC_PRN(PRN_INFO,"\n write original data to block 0x%x ", start_blk);
        rtn=EMMC_WriteBlk( start_blk*0x200, buf_wr, 0x200);


        //read and check original data
        memset(buf_rd,0,512);//clear read buffer
        EMMC_PRN(PRN_INFO,"\n read inverted data from block 0x%x ", start_blk);
        rtn=EMMC_ReadBlk((long long)start_blk*0x200, buf_rd, 0x200);
        ...
        if((i%10000)==0&&i!=0)
        	EMMC_PRN(PRN_RES, "blk[%d]",start_blk) ;

        if (rtn == 1)
        	break;
        else
        	EMMC_PRN(PRN_INFO,"... Pass\n");
        start_blk++;
    }

    if(rtn == 1){
        EMMC_PRN(PRN_ERR, "\n\nSequential Write/Read test failed at block %d !\n\n",start_blk) ;
        //	dump the read and write data
        EMMC_PRN(PRN_INFO,"............. Data in Write buffer ...........\n");
        for(i=0; i<512; i++){
        	EMMC_PRN(PRN_INFO, "0x%02x\t", *((unsigned char *)(buf_wr+i)) ) ;
    ...
    if (0 == rtn)
        EMMC_PRN(PRN_RES, "\n Sequential Walk through sector%d ~ sector%d Passed \n",start_blk-cnt,start_blk);
    else
    	EMMC_PRN(PRN_RES,"\n flash Failed \n");
    return rtn;
}

UINT_T cmd_EMMC_VRLOCK(char *params_str)
{
    EMMC_Get_Extcsd();

    //byte[3]=CSD[13],byte[2]=CSD[12],byte[1]=EXT_CSD[173],byte[0]=EXT_CSD[171]
    EMMC_PRN(PRN_INFO,"g_emmc_WP_info=0x%X\n",g_emmc_WP_info);
    if(g_emmc_WP_info==0)
        EMMC_PRN(PRN_RES,"Boot area: Not Write Protected\nUser area: Not Write Protected\n");
    else
    {
        if(g_emmc_WP_info&0x1000004==0x1000000)
        	EMMC_PRN(PRN_RES,"Boot area: Permanently Write Protected\nUser area: Permanently Write Protected\n");
        else
        {
        	EMMC_PRN(PRN_RES,"Boot area: ");
        	if(g_emmc_WP_info&0x1400==0x400)
        		EMMC_PRN(PRN_RES,"Permanently Write Protected\n");
        	else if(g_emmc_WP_info&0x4100==0x100)
        		EMMC_PRN(PRN_RES,"Power-on Write Protected\n");
        	else if(g_emmc_WP_info&0x00010000==0x00010000)
        		EMMC_PRN(PRN_RES,"Temporarily Write Protected\n");
        	else
        		EMMC_PRN(PRN_RES,"Not Write Protected\n");

        	EMMC_PRN(PRN_RES,"User area: ");
        	if(g_emmc_WP_info&0x14==0x4)
        		EMMC_PRN(PRN_RES,"Permanently Write Protected\n");
        	else if(g_emmc_WP_info&0x9==0x1)
        		EMMC_PRN(PRN_RES,"Power-on Write Protected\n");
        	else if(g_emmc_WP_info&0x00010000==0x00010000)
        		EMMC_PRN(PRN_RES,"Temporarily Write Protected\n");
        	else
...
int cmd_EMMC_WRITE(char *params_str )
{
    int rtn = 0;
    unsigned long long start_blk;
    unsigned int blk;
    unsigned char *puchWriteBuff;

    DIAG_PARAM_PARSER_HEX(params_str, start_blk,     0);
    DIAG_PARAM_PARSER_HEX(params_str, blk,     0);
    DIAG_PARAM_PARSER_HEX(params_str, puchWriteBuff,     SDIO_WD_BUF);
    rtn = EMMC_WRITE(start_blk, blk, puchWriteBuff);
    return rtn;
}
...
/*  hex, &variable,            "name",          "info" */
    {1, &autocmd12,       "AUTOCMD12",        "use autoCMD12 for CMD18/CMD25"},
};


int cmd_EMMC_PARAM(char *params_str)
{
    return diag_cmd_PARAM(params_str, params, ARRAY_NUM(params));
}


/****************************************************************************
 DIAG CMD array. (keep this section at end of file)
****************************************************************************/
#define DIAG_CMD(command, help)                     {#command, help, cmd_EMMC_##command}

const diag_cmd_entry_t    emmcDiag_cmds[] =
{
    DIAG_CMD( ADMA,    "[write] [read]; ADMA enable for write and read"),
    DIAG_CMD( AUTOCMD12,    "[write]; enable/disable autocmd12 when write/read multiple blocks"),
    DIAG_CMD( BOOTPARTITION,    "[Partition] [Size_In_KB] use alternative boot mode to read [Size_In_KB] data from [Partition]"),
    DIAG_CMD( CLOCK,    "[start:0->stop, 1->start] [clock divider]; set SDIO clock divider"),
    DIAG_CMD( CMD,    "[cmd]; send SDIO individual commmand"),
    DIAG_CMD( DRIVE,    "[type 0~4],select drive type"),
    DIAG_CMD( DUMP,    "dump all emmc register"),
    DIAG_CMD( ERASE,    "[start block] [block count]; start SDIO erase test"),
    DIAG_CMD( EXTCSD,    "read EXT_CSD registers in eMMC device"),
    DIAG_CMD( GETID,    "read device Manufacturer ID an Product Serial Number"),
    DIAG_CMD( INIT, "[]; initinalize"),
    DIAG_CMD( INTR, "[1->enable interrupt, 0->disable]; turn on/off SDIO interrupt"),
    DIAG_CMD( M2M,    "[start block] [iteration]; start SDIO m2m test"),
    DIAG_CMD( M2MEX,    ""),
    DIAG_CMD( MODE,    "[Mode];change eMMC Mode; 0->HS SDR mode, 1->HS DDR mode, 2->HS200, 3->HS400"),
    DIAG_CMD( PARAM,    "[name] [value]; set/list test parameters"),
    DIAG_CMD( PARTITION,    "[Partition] switch to [partition]"),
    DIAG_CMD( PERFORMANCE,    "[Number of blocks][Iterations] read/write number of blocks to test the read/write performance of SDIO"),
    DIAG_CMD( READ,    "[start block] [block count]; start SDIO read test"),
    DIAG_CMD( RESET,    "[]; reset SDIO HC"),
    DIAG_CMD( RSTENABLE,    "[];permanently enable RST_n signal"),
    DIAG_CMD( SECM2M,    "[start block] [block count]; start SDIO secure memort m2m test"),
    DIAG_CMD( SEQTEST,    "[start block] [num of blocks];SDIO sequential write/read test (block by block)"),
    DIAG_CMD( SLEEP,    "[sleep/wakeup, 0: wakeup, 1: sleep, 2: check]; emmc sleep/wakeup"),
    DIAG_CMD( START,    "[]; Card identification"),
    DIAG_CMD( TEST,    "[cmdIdx] [argument] [respType 0~3]; send SD command for test purpose"),
    DIAG_CMD( TUNE,    " tune for HS200&HS400"),
    DIAG_CMD( TXCFG,    "[Degree phase shift];(0x0~0x1F) 0->45 degree, 10->90 degree, 1F->135 degree"),
    DIAG_CMD( VRSEC,    "[start block] [block count];read->write inverted->read->write inverted test block by block"),
    DIAG_CMD( VRLOCK,    "[];Verify write protection for boot area and user area"),
    DIAG_CMD( WRITE,    "[start block] [block count]; start SDIO write test"),

};

int parseEmmcCmd(char *pCmdLine)
{
    return diag_cmdParser(pCmdLine, emmcDiag_cmds, ARRAY_NUM(emmcDiag_cmds), NULL);
}
...
int regression_emmc(void)
{
    ...
    Ret = cmd_EMMC_START();
    if (Ret == 0)
        EMMC_PRN(PRN_RES, "\n eMMC Card initialization done.\n");
    else
    {
        EMMC_PRN(PRN_RES, "\n eMMC Card initialization failed. Retrying....\n");
        cmd_EMMC_START();
    }
    if(Ret==0)
    {
        sprintf(params_str, "%x %x %x %x", 0x100, 5, 0xA00000, 0xB00000);
        Ret = cmd_EMMC_M2M(params_str); //change the write&read buffer address to avoid overlap
    }
    return Ret;
}
#endif
