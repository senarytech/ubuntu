...
def copy_non_android_partitions(dst_emmc, target_files):
    ...
    try:
        ...
        for image in images:
            dst_emmc_img = os.path.join(dst_emmc, image[:-4] + ".subimg")
            shutil.copy(os.path.join(temp_dir, 'RADIO', image), dst_emmc_img)
    ...
    try:
       ...
       # uncompress the IMAGES partitions from the target_files zip
        with open(os.devnull) as out:
           ...
           images = ['boot', 'dtbo','vbmeta', 'vbmeta_system', 'vendor_boot']
...
def copy_missing(src_emmc, dst_emmc):
    ...
    files = ['emmc_image_list', 'emmc_part_list', 'emmc_image_list_full', 'userdata.subimg', 'boot-debug.subimg', 'vendor_boot-debug.subimg']
...
def patch_emmc(src_emmc, dst_emmc, target_files, key_data, avb_key):
    ...
    try :
        ...
        # patch key images
        patch_key_image(src_emmc, dst_emmc, 'key.subimg', key_data, avb_key)
