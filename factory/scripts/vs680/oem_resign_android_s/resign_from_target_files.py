...
def patch_zip(input_zip, output_zip, key_data):
    ...
    try:
        ...
        patch_nw_bootloader(temp_dir, 'bl_normal.subimg', key_data)
        patch_nw_bootloader(temp_dir, 'bl_recovery.subimg', key_data)

        # patch fastboot
        patch_fastboot(temp_dir, 'fastboot_1st.subimg', key_data)
        patch_fastboot(temp_dir, 'fastboot_2nd.subimg', key_data)

        # update the copy of the recovery nw bootloader that is stored in the system partition
        shutil.copy(os.path.join(temp_dir, 'RADIO', 'bl_recovery.subimg'),
                    os.path.join(temp_dir, 'VENDOR', 'etc/boot', 'bl_recovery.subimg'))

        # patch tzk
        patch_tzk(temp_dir, 'tzk_normal.subimg', key_data)
        patch_tzk(temp_dir, 'tzk_recovery.subimg', key_data)

        shutil.copy(os.path.join(temp_dir, 'RADIO', 'tzk_recovery.subimg'),
                    os.path.join(temp_dir, 'VENDOR', 'etc/boot', 'tzk_recovery.subimg'))
...
def main():
    ...
    input_keys = { 'new_oem_custk' : 'File containing the new raw OEM_CUSTK key to be used reencrypt images',
                   ...
                   'root_signk_store': 'Store containing the new root RSA signk7 store that will be replaced in preboot image', }
