...
SIMG2SIMG_TOOL = 'simg2simg'
IMG_FROM_TARGET_FILES = 'out/host/linux-x86/bin/img_from_target_files'
BUILD_SUPER_IMAGE = 'out/host/linux-x86/bin/build_super_image'
SPARSE2RAW_TOOL = 'simg2img'
VERBOSE = True
...
def copy_non_android_partitions(dst_emmc, target_files):
    ...
    try:
        ...
        images = [ 'bl_normal.subimg', 'bl_recovery.subimg', 'tzk_normal.subimg', 'tzk_recovery.subimg', 'fastboot_1st.subimg', 'fastboot_2nd.subimg' ]

        for image in images:

            shutil.copy(os.path.join(temp_dir, 'RADIO', image), dst_emmc)
    ...
    try:
       ...
       # uncompress the IMAGES partitions from the target_files zip
        with open(os.devnull) as out:
           ...
           images = ['boot', 'dtbo','vbmeta', 'vbmeta_system', 'recovery', 'vendor_boot']
        ...
        try:
           ...
           full_path = os.path.join(my_dir, SIMG2SIMG_TOOL)
           subprocess.check_call([full_path, super_image, super_image, '524288000'])
           os.remove(super_image)
        except subprocess.CalledProcessError:
...
def copy_missing(src_emmc, dst_emmc):
    ...
    files = ['emmc_image_list', 'emmc_part_list', 'emmc_image_list_full', 'userdata.subimg', 'boot-debug.subimg', 'cache.subimg', 'vendor_boot-debug.subimg']
...
def patch_emmc(src_emmc, dst_emmc, target_files, key_data, avb_key):
    ...
    if not os.path.exists(SIMG2SIMG_TOOL):
        raise Exception('Cannot find tool %s ' % SIMG2SIMG_TOOL)
    ...
    try :
        ...
        # patch key images
        patch_key_image(src_emmc, dst_emmc, 'key_1st.subimg', key_data, avb_key)
        patch_key_image(src_emmc, dst_emmc, 'key_2nd.subimg', key_data, avb_key)
