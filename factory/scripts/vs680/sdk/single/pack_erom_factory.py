...
def usage():
	print 'Usage:'
	print '  %s ROMK_FACTORY_EROM\n' % sys.argv[0]
	print '  ROMK_FACTORY_EROM: input, ROMK encrypted factory EROM'
...
def check_custk_store_image(input_image):
    ...
    with file(input_image, 'r+b') as input_file:
        ...
        if int(check_bytes.encode('hex'), 16) != CUSTKSTORE_CHECK_PATTERN:
            ...
            print  'EROM Factory is wrong format or not ROMK'
...
if __name__ == '__main__':
	...
	with open('./imglst.json', 'w+') as jsonfile:
		jsonfile.write('[\n')
		output = COMMON_PREFIX + filename + '", "type": "eromf",  "path": "./"},\n'
