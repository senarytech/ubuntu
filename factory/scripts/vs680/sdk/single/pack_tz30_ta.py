...
def usage():
	print 'Usage:'
	print '  %s ROMK_TA\n' % sys.argv[0]
	print '  ROMK_TA: input, ROMK encrypted TZ30 TA'
...
def check_ta_image(ta_image):
    ...
    with file(ta_image, 'r+b') as ta_file:
        ...
        if header != TA30_CHECK_MAGIC_NUM or int(check_bytes.encode('hex'), 16) != CUSTKSTORE_CHECK_PATTERN:
...
if __name__ == '__main__':
	...
	with open('./imglst.json', 'w+') as jsonfile:
		jsonfile.write('[\n')
		output = COMMON_PREFIX + filename + '", "type": "ta30",  "path": "./"},\n'
