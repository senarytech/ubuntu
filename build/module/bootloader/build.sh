...
build_bootloader() {
  ...
  if [ "is${CONFIG_BOOT_SPI_THEN_SD}" = "isy" ]; then
      cmd2run="${cmd2run} CONFIG_BOOT_SPI_THEN_SD=y"
  fi
  if [ "is${CONFIG_GEN_SKIP}" = "isy" ]; then
    cmd2run="${cmd2run} CONFIG_GEN_SKIP=y"
  fi
  ...
  if [ "is${CONFIG_BL_EXTERNAL_PLATFORM}" = "isy" ];then
        cmd2run="${cmd2run} PLATFORM=${CONFIG_BL_EXTERNAL_PLATFORM_NAME}"
        cmd2run="${cmd2run} PLATFORM_DIR=${CONFIG_EXTERNAL_ROOT_PATH}/${CONFIG_BL_EXTERNAL_PLATFORM_PATH}"
        cmd2run="${cmd2run} CONFIG_EXTERNAL_PLATFORM=y"
  else
...
if [ "is${CONFIG_PREBOOT_BOOTFLOW_VERSION}" = "isold" ]; then
  ...
  if [ "is${CONFIG_GENX_ENABLE}" = "isy" ]; then
    ...
    cp ${opt_outdir_intermediate}/bootloader_raw.bin ${opt_outdir_release}/bootloader_en.bin
  else
