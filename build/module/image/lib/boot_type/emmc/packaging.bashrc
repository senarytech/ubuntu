...
emmc_split_image() {
  ...
  subimg_name=$1; shift
  Split_size=$1; shift
  ${basedir_tools}/img2simg_android_x86_64 ${outdir_subimg_intermediate}/${subimg_file}.subimg ${outdir_subimg_intermediate}/${subimg_file}_s.subimg 4096
  ${basedir_tools}/simg2simg_android_x86_64 ${outdir_subimg_intermediate}/${subimg_file}_s.subimg ${outdir_subimg_intermediate}/${subimg_file}.subimg ${Split_size}

  index=0
  while [ -f ${outdir_subimg_intermediate}/${subimg_file}.subimg.${index} ]
  do
    cat ${outdir_subimg_intermediate}/${subimg_file}.subimg.${index} | gzip -1 > ${outdir_product_release_emmc}/${subimg_name}.subimg.${index}.gz
    index=$((index+1))
  done

  cd ${outdir_product_release_emmc}
  image_list=`ls ${subimg_name}.subimg.*.gz`
  pattern_s=`grep ${subimg_name} emmc_image_list`
  pattern_r=$pattern_s
  for s_index in $image_list; do
...
emmc_release_subimg() {
  ...
  subimg_file=$1; shift
  subimg_name=$1; shift
  Split_size=943718400

  [ -f ${outdir_subimg_intermediate}/${subimg_file}.subimg ]

  size=`stat --format=%s ${outdir_subimg_intermediate}/${subimg_file}.subimg`

  if [ ${size} -gt ${Split_size} ]; then
    emmc_split_image "${subimg_file}" "${subimg_name}" ${Split_size}
  else
 ...
 # cp ${outdir_subimg_intermediate}/${subimg_file}.subimg ${outdir_product_release_emmc}/${subimg_name}.subimg
}
...
#
### Fastboot ###
if [ "is${CONFIG_UBOOT_FASTBOOT}" == "isy" ] || [ "is${CONFIG_UBOOT_BOOTUP_SYSTEM}" == "isy" ]; then
...
### Preload TA ###
if [ "is${CONFIG_BL_PRELOAD_TA}" = "isy" ] || [ "is${CONFIG_UBOOT_BOOTUP_SYSTEM}" == "isy" ]; then
