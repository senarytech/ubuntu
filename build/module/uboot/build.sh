...
build_uboot() {
  ...
  eval "$cmd2run make mrproper"
  eval "$cmd2run make ${CONFIG_UBOOT_DEFCONFIG} -j${CONFIG_CPU_NUMBER}"
  eval "$cmd2run make EXT_DTB=arch/${CONFIG_UBOOT_ARCH}/dts/${CONFIG_UBOOT_DTS}.dtb -j24"
  popd
}
#############
# Functions #
...
gen_spi_combo() {
  ...
  if [ "${syna_chip_name}" = "bg4ct" ]; then
    ...
    if [ "is${CONFIG_PREBOOT_DDR_PHY_FW}" = "isy" ]; then
      if [ "is${CONFIG_BOOT_SPI_THEN_SD}" = "isy" ]; then
        cat $f_erom $f_sysinit $f_ddrphyfw $f_miniloader $f_tsm >> $f_spi_combo
      else
...
gen_x_spi_combo() {
  ...
  elif [ "is${CONFIG_PREBOOT_DDR_PHY_FW}" = "isy" ] && [ "is${syna_chip_name}" = "isvs680" ]; then
...
if [ "is${CONFIG_UBOOT_FASTBOOT}" != "isy" ]; then
  if [ "is${CONFIG_UBOOT_SPIUBOOT}" != "isy" ]; then
    if [ "is${CONFIG_UBOOT_BOOTUP_SYSTEM}" != "isy" ]; then
      echo "select uboot action is error!"
      exit 0
    fi
  fi
fi
...
if [ "is${CONFIG_UBOOT_FASTBOOT}" == "isy" ] || [ "is${CONFIG_UBOOT_BOOTUP_SYSTEM}" == "isy" ]; then
 ...
 if [ "is${CONFIG_GENX_ENABLE}" = "isy" ]; then
  ...
  elif [ "is${CONFIG_GEN_SKIP}" = "isy" ]; then
    cp ${opt_outdir_intermediate}/uboot_raw.bin ${opt_outdir_release}/uboot_en.bin
  else
