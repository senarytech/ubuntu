...
if [ "x${opt_boot_spi_then_sd}" != "x" ]; then
  cmd_args="${cmd_args} --config_boot_spi_then_sd=${opt_boot_spi_then_sd}"
fi

if [ "x${opt_gen_skip}" != "x" ]; then
  cmd_args="${cmd_args} --config_gen_skip=${opt_gen_skip}"
fi
