...
if [ "is${CONFIG_BOOT_SPI_THEN_SD}" = "isy" ]; then
    opt_boot_spi_then_sd=${CONFIG_BOOT_SPI_THEN_SD}
fi

if [ "is${CONFIG_GEN_SKIP}" = "isy" ]; then
    opt_gen_skip=${CONFIG_GEN_SKIP}
fi
...
if [ "is${opt_genx_enable}" != "is" ]; then
  ...
  if [ "is${syna_chip_name}" = "isvs640" ]; then
    dd if=/dev/zero of=${preboot_outdir_build_release}/ddrphy.bin bs=1 count=98424
  elif [ "is${opt_config_ddrphyfw}" = "isy" ] && [ "is${opt_chip_name}" = "isvs680" ]; then
    ##FIXME: will add VS680 ddrphy image handle here.
    dd if=/dev/zero of=${preboot_outdir_build_release}/ddrphy.bin bs=1 count=98424
  fi
...
if [ "is${opt_production_build}" = "isy" ]; then
  ...
  if [ "is${opt_boot_spi_then_sd}" = "isy" ]; then
    if [ "is${opt_gen_skip}" = "isy" ]; then
      f_miniloader=${preboot_prebuilts_dir_bootflow}/miniloader_sd_gen_skip_en_mp.bin
    else
      f_miniloader=${preboot_prebuilts_dir_bootflow}/miniloader_sd_en_mp.bin
    fi
  else
    if [ "is${opt_gen_skip}" = "isy" ]; then
      f_miniloader=${preboot_prebuilts_dir_bootflow}/miniloader_gen_skip_en_mp.bin
    else
     f_miniloader=${preboot_prebuilts_dir_bootflow}/miniloader_en_mp.bin
    fi
  fi
  ...
  if [ "is${opt_boot_spi_then_sd}" = "isy" ]; then
    if [ "is${opt_gen_skip}" = "isy" ]; then
      f_miniloader=${preboot_prebuilts_dir_bootflow}/miniloader_sd_gen_skip_en.bin
    else
     f_miniloader=${preboot_prebuilts_dir_bootflow}/miniloader_sd_en.bin
    fi
  else
    if [ "is${opt_gen_skip}" = "isy" ]; then
      f_miniloader=${preboot_prebuilts_dir_bootflow}/miniloader_gen_skip_en.bin
    else
      f_miniloader=${preboot_prebuilts_dir_bootflow}/miniloader_en.bin
    fi
  fi
