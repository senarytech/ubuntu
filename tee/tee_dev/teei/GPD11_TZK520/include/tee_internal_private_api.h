 ...
 *
 * Here are the shm use cases.
 * * Case : create generic shm with other TA ReadOnly and physical continuous
 *   - Client TA (SHM Owner)
 *     . TEE_AllocateSharedMemory(size, TEE_SHM_PEER_READ |
 *     			TEE_SHM_PHYSICAL_CONTINUOUS, NULL, &shm, &va)
 *     . TEE_RetrieveSharedMemoryID(shm, &shmid)
 *     . TEE_InvokeTACommand(session,...) with shmid
 *     . or TEE_OpenTASession(...) with shmid as parameters
 *     . TEE_ReleaseSharedMemory(shm);
 *   - Service TA, in TA_OpenSessionEntryPoint or TA_InvokeCommandEntryPoint
 *     . TEE_RegisterSharedMemory(shmid, &shm, &va)
 *     . TEE_InvokeTACommand(otherSession,...) use shmid to share with other TA
 *     . TEE_ReleaseSharedMemory(shm);
 */
 ...
 *
 * @param key		shared memory key. 0 for private.
 *			if it's not 0, it tries to find the shared memory with
 *			associated key first.
 * @param len		shared memory size in bytes.
 * @param shmFlags	permission control, see TEE_SHM_*.
 * @param param		parameter for differnet flags.
 *			for TEE_SHM_SESSION_PRIVATE/TEE_SHM_SESSION_TA_PRIVATE,
 *			it can be:
 *			- session handle: peer is the service TA.
 *			- NULL: peer is the client TA. If client is non-secure
 *				world, then it will get an error.
 * @param shm		buffer to return the shm handle.
 * @param va		buffer to return shm virtual address.
 *
 * @retval TEE_SUCCESS			success.
 * @retval TEE_ERROR_BAD_PARAMETERS	invalid parameters.
 * @retval TEE_ERROR_OUT_OF_MEMORY	out of memory.
 */
TEE_Result TEE_AllocateSharedMemory(uint32_t key, size_t len,
		uint32_t shmFlags, void *param,
		TEE_ObjectHandle *shm, void **va);

/** release secure shared memory.
 *
 ...
 *
 * must call TEE_ReleaseSharedMemory() to release it after use it.
 *
 * @param shmid		Shared memory ID shared from other TA instance.
 * @param shm		buffer to return the shm handle.
 * @param va		buffer to return shm virtual address.
 * @param size		buffer to return shared memory size in bytes.
 *
 * @retval TEE_SUCCESS			success.
 * @retval TEE_ERROR_BAD_PARAMETERS	shm == NULL
 * @retval TEE_ERROR_ITEM_NOT_FOUND	the shared memory ID is invalid.
 */
TEE_Result TEE_RegisterSharedMemory(uint32_t shmid,
		TEE_ObjectHandle *shm, void **va, size_t *size);
