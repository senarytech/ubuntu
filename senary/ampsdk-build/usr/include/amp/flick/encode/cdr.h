...
#define flick_cdr_encode_stringlen(_data, _link, _onerror) {		\
		flick_##_link##_encode_new_glob(7, _onerror);				\
	flick_##_link##_encode_new_chunk_align(4, 2, 0, 0);		\
	flick_##_link##_check_span(4, _onerror);			\
	flick_cdr_encode_unsigned32(0, _data, unsigned int);		\
	flick_##_link##_encode_end_chunk(4);				\
	flick_##_link##_encode_end_glob(7);				\
}
...
#define flick_cdr_encode_fd(_ofs, _data, dest_type) \
    flick_cdr_encode_fd_internal(_stream, _data, (char*)((char*)_buf_current+_ofs))

#define flick_cdr_decode_fd(_ofs, _data, dest_type) \
    _data = flick_cdr_decode_fd_internal(_stream, (char*)((char*)_buf_current+_ofs))

#ifndef TRAPEZE
#define flick_cdr_encode_IOR(_link, _data, _ref_adj, _onerror)		\
	if( flick_cdr_encode_IOR_internal(_stream,	&_stub_state,		\
				       (_data),				\
				       (#_link),			\
				       (_ref_adj)) )			\
	     flick_stub_error(FLICK_ERROR_NO_MEMORY, _onerror);
...
#define flick_cdr_decode_IOR(_link, _data, _ref_adj)			\
	(_data) = flick_cdr_decode_IOR_internal(_stream, &_stub_state,	\
						0 /* no swap */,	\
						(#_link),		\
						(_ref_adj))
