...
#define flick_iiop_client_start_encode() {				     \
	...
	((int *) _buf_start)[4] = _request_state.seqnum;			     \
...
#define flick_iiop_server_start_decode() {				 \
	_stream = InHeadP;						 \
	printf("start decode: request_id = %d\n", _request_id); \
...
#define flick_iiop_server_start_encode() {				\
	...
	printf("start encode: request_id=%d\n", _request_id); \
...
int flick_client_start_request(FLICK_TARGET obj, CORBA_BOA boa, struct flick_request_state *request_state);
int flick_client_wait_request(FLICK_TARGET obj, struct flick_request_state *request_state);
int flick_client_finish_request(FLICK_TARGET obj, CORBA_BOA boa, struct flick_request_state *request_state);
