...
#define SONY_TRACE_RETURN(result) \
    SONY_MACRO_MULTILINE_BEGIN \
        sony_trace_log_return((result), __FUNCTION__, __LINE__); \
        return (result); \
    SONY_MACRO_MULTILINE_END    

#define TunerDemodLog(...) {printf("%s:%d: ", __FUNCTION__, __LINE__); printf(__VA_ARGS__);}
...
#define SONY_TRACE_I2C_RETURN(result) \
    SONY_MACRO_MULTILINE_BEGIN \
        sony_trace_i2c_log_return((result), __FUNCTION__, __LINE__); \
        return (result); \
    SONY_MACRO_MULTILINE_END
