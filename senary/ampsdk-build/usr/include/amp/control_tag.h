 ...
 */
#ifndef _CONTROL_TAG_
#define _CONTROL_TAG_
...
typedef struct t_mv_timestamp {
    UINT    m_StampHigh;    // general time stamp high bits, highest bit set to
                            // 1 to indicate time stamp valid.
...
//!         discontinuity within one single av stream file. It means that all time info after this
//!         point is on a new time base.
//!c)    it¡¯s un-announced PCR discontinuity, detected by DMX module (in the dtv case). In this
//!         case, the eBS_DiscontIndicate control tag is issued when a new steady time base
//!         is established. At least 3 consecutive PCR is needed to establish a new time base.
//!         So the position(data count) indicated in the control tag could be up to several
//!         hundred milisecond behind the real change position. If you need more
//!         precise info on the time base change, you can use the following two
//!         new control tags.
...
//! eBS_PcrJump will be issued as soon as a PCR out of proper range is received.
//! It could be caused by a time base change, but also could be input data error.
//! DMX need to monitor the several PCR after the first error one. After a steady
//! time base is established again, eBS_TimeBaseNotif control tag will be output
//! to every output channel. If the new established time base is a different time
//! base from the original one, m_IsNewTimeBase in the control tag will be set
//! to 1. Otherwise, m_IsNewTimeBase=0.
