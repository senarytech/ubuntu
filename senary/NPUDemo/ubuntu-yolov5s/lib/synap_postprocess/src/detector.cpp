...
{
    float w_ratio=in_dim.x/640.0;
    float h_ratio=in_dim.y/640.0;
    ...
    for(const Tensor& tensor: tensors) {
        ...
        for (int32_t i = 0; i < num_boxes; i++, detections += det_sz) {
           ...
           // cout << "  i =  "<< i << "  detection->x =  "<< detection->x << "  detection->y =  " << detection->y <<  "  detection->w = "<< detection->w << "  detection->h =  "<< detection->h << endl;

          // cout <<  "  x =  "<< detection->x << "   y=  "<<  detection->y  << "   w =  "<<  detection->w  << "    h =  "<< detection->h <<endl;


            Box box;
            box.tl.x =int((detection->x - detection->w / 2) * w_ratio);  //w_ratio
            box.tl.y =int((detection->y - detection->h / 2) );   //h_ratio
            box.br.x =int((detection->x + detection->w / 2)*w_ratio );   //w_ratio
            box.br.y =int((detection->y + detection->h / 2)* h_ratio);  //h_ratio
            
          //  cout << "************ " <<endl;
	   // cout <<  "  box.tl.x =  "<< box.tl.x << "   box.tl.y =  "<<  box.tl.y  << "   box.br.x =  "<< box.br.x  << "    box.br.y =  "<< box.br.y <<endl;
...
Detector::Result Detector::process(const Tensors& tensors, const Rect& input_rect)
{
    ...
    Timer tmr;
    Dim2d rect_size = input_rect.size;
    cout << "  input_rect.size  =  " << input_rect.size  << "    input_rect.x  =  " << rect_size.x  << "  input_rect.y  =  " << rect_size.y << endl;
