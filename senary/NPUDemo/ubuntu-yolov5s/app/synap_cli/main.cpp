...
#ifdef ENABLE_ION_ALLOCATOR
    "ion", AllocatorType::ion,
#endif
...
static InputData get_input_data(const string& input_filename, size_t size)
{
    ...
    if (filename == "random") {
        ...
        return InputData(move(random_data), InputType::raw);
    }
    ...
    if (filename.find(numstr_prefix) == 0) {
        ...
        return InputData(move(value), InputType::raw);
    }
...
#ifdef ENABLE_ION_ALLOCATOR
        { AllocatorType::ion, ion_allocator, ion_secure_allocator, ion_cma_allocator, ion_cma_secure_allocator },
#endif
#ifdef ENABLE_OSAL_ALLOCATOR
...
int main(int argc, char** argv)
{
    ...
    string nb_filename = args.get("--nb", "<file> Network binary model file", "model.nb");
    string meta_filename = args.get("--meta", "<file> Network meta file (json)", "model.json");
    ...
    for(int nc = 0; nc < num_create; nc++) {
        ...
        Timer load_tmr;
        vector<uint8_t> ebg = binary_file_read(nb_filename);
        if (ebg.empty()) {
            cerr << "Failed to load ebg file: " << nb_filename << endl;
            return 1;
        }
        string ebg_meta = file_read(meta_filename);
        if (ebg_meta.empty()) {
            cerr << "Failed to load metafile: " << meta_filename << endl;
            return 1;
        }
        ...
        Network net;
        Timer init_tmr;
        if (!net.load_model(ebg.data(), ebg.size(), ebg_meta.c_str())) {
        ...
        for (Tensor& tensor : net.inputs) {
            ...
            input_data.push_back(move(data));
